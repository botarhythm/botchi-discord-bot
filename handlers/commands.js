...
async function handleReset(args, message) {
  try {
    ...
    if (config.DM_MESSAGE_HANDLER === 'new') {
      // 新しいプロバイダーシステムを使用
      if (!aiService || !aiService.getProvider) {
...
async function handleIntervention(args, message) {
  ...
  if (args[0].toLowerCase() === 'cooldown') {
    ...
    // コンテキスト介入ハンドラーを使用してクールダウンを更新
    const updated = contextIntervention.updateInterventionCooldown(seconds);
    
    if (updated) {
      await message.reply(`文脈介入のクールダウンを ${seconds}秒 に設定しました。`);
    } else {
      await message.reply('クールダウンの更新に失敗しました。');
    }
    return;
  }
  ...
  // コンテキスト介入ハンドラーを使用してモードを更新
  const updated = contextIntervention.updateInterventionMode(newMode);
  
  if (updated) {
    await message.reply(`文脈介入モードを「${newMode}」に設定しました。`);
  } else {
    await message.reply('モードの更新に失敗しました。');
  }
}
